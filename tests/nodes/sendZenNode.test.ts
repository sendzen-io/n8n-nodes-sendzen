...
describe('SendZen Node', () => {
	...
	test('should have the correct Properties and length', () => {
		...
		expect(properties[0]).toBe(OperationProperties);
		expect(properties[1]).toBe(WabaAccountProperties);
		expect(properties[2]).toBe(PhoneNumberIdProperties);
		expect(properties[3]).toBe(MessageIdProperties);
		expect(properties[4]).toBe(RecipientPhoneNumberProperties);
		expect(properties[5]).toBe(TemplateProperties);
		expect(properties[6]).toBe(ReplyMessageProperties);
		expect(properties[7]).toBe(EnableUrlPreviewProperties);
		expect(properties[8]).toBe(TemplateNameOrIdProperties);
	});
	...
	test('should have the correct Methods', () => {
		...
		let loadOptions = methods?.loadOptions;
		if(loadOptions){
			let loadOptionsKeys = Object.keys(loadOptions);
		...
		let resourceMapping = methods?.resourceMapping;
		if(resourceMapping){
			let resourceMappingKeys = Object.keys(resourceMapping);
